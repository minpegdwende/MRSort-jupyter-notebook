...
class LpMRSortWeights(object):
    ...
    def __init__(self, model, pt, aa_ori, delta=0.0001, gamma = 0.5, renewal_method = 2, pretreatment_crit = None,
    ...
    def update_linear_program(self,fct_w_threshold = 0, nb_unk_criteria = 1):
        self.fct_w_threshold = fct_w_threshold
...
#                self.lp.set_error_stream(None)
    ...
    def add_variables_cplex(self):
        # w with potential limits
        #print(self.it_meta)
#        if self.it_meta > 20:
#            self.w1_threshold = 0
        
        #print(self.it_meta,self.w1_threshold)
        self.lp.variables.add(names=['w'+c.id for c in self.model.criteria],
                              lb=[(self.fct_w_threshold if int(c.id[1:]) <= self.nb_unk_criteria else 0) for c in self.model.criteria],
        ...
        self.lp.variables.add(names=['yp'+dj for dj in self.c_yi],
                              lb = [0 for dj in self.c_yi],
                              ub = [1 for dj in self.c_yi])
        # modification of lambda interval
        self.lp.variables.add(names=['lambda'], lb = [0.5], ub = [1])
        #import pdb; pdb.set_trace()
#        self.lp.variables.add(names=['eta'], lb = [0], ub = [1]) # for renewal_method V30,32,..
#        self.lp.variables.add(names=['eta'+c.id for c in self.model.criteria],
#                              lb=[0 for c in self.model.criteria],
#                              ub=[1 for c in self.model.criteria])        
#        self.lp.variables.add(names=['etap'+c.id for c in self.model.criteria],
#                              lb=[0 for c in self.model.criteria],
#                              ub=[1 for c in self.model.criteria])
        #control the born of weigths here !!!
    ...
    def add_constraints_cplex(self):
        ...
        #constraint w1 - eta1 = 0.05
#        for c in self.model.criteria:
#            var = c.id
#            constraints.add(names=[var+"_thresh"],
#                                lin_expr = [[ ['w'+var] + ['eta'+var],
#                                            [1.0, -1.0]],
#                                           ],
#                                senses = ["E"],
#                                rhs = [self.w_threshold]
#                                )
#            constraints.add(names=[var+"p_thresh"],
#                                lin_expr = [[ ['w'+var] + ['etap'+var],
#                                            [1.0, 1.0]],
#                                           ],
#                                senses = ["E"],
#                                rhs = [1-self.w_threshold]
#                                )
    
        #sonstraint w1 - eta = 0.1
        #import pdb; pdb.set_trace()

#        if self.addition_w1_constraint :
#            constraints.add(names=['w1_pref_dir'],
#                                lin_expr = [[ [w_vars[0]] + ['eta'],
#                                            [1.0, -1.0]],
#                                           ],
#                                senses = ["E"],
#                                rhs = [0.1]
#                                )
        ...
        if self.renewal_method in [-2]:
            # additionnal constraint to prune more when having duplicated criteria
            # sum w_j - w_j_dupl = 0
            w_vars_dupl = set(list(map(lambda x: tuple(sorted(['w'+x.id, 'w'+x.dupl_id])) , filter(lambda x: x.dupl_id != None, self.model.criteria))))
            #import pdb; pdb.set_trace()
            for elem in w_vars_dupl:
                constraints.add(names=['wdupl' + elem[0]],
                                lin_expr = [[ [elem[0],elem[1]],
                                            [1.0, -1.0]],
                                           ],
                                senses = ["E"],
                                rhs = [0]
    ...
    def add_objective_cplex(self):
        ...
        if self.renewal_method in [1, 2, 3]:
        ...
        if self.renewal_method in [-3]: # additionnal objective to force w_dupl to 0
            for dj, coef in self.c_xi.items():
                self.lp.objective.set_linear('xp'+dj, coef * (1-self.gamma)) # * 10)
            for dj, coef in self.c_yi.items():
                self.lp.objective.set_linear('yp'+dj, coef * (1-self.gamma))
            w_vars_dupl = set(list(map(lambda x: tuple(sorted(['w'+x.id, 'w'+x.dupl_id])), filter(lambda x: x.dupl_id != None, self.model.criteria))))
            #import pdb; pdb.set_trace()
            for elem in w_vars_dupl:
                self.lp.objective.set_linear(elem[0], self.gamma)
                self.lp.objective.set_linear(elem[1], self.gamma)
            #import pdb; pdb.set_trace()
            
#        if self.renewal_method == 4: # additionnal objective maximize w+w_dupl to 0
#            for dj, coef in self.c_xi.items():
#                self.lp.objective.set_linear('xp'+dj, coef * (1-self.gamma) * 10)
#            for dj, coef in self.c_yi.items():
#                self.lp.objective.set_linear('yp'+dj, coef * (1-self.gamma))
